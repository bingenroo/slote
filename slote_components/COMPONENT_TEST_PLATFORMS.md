...
This document describes the standalone test platforms created for each component in `slote_components/`. These test apps enable independent development and debugging of components without needing to run the full Slote application.

## Purpose

Previously, testing components required running the entire `slote_app/`, which included:
...
**Location**: `slote_components/slote_draw/example/`
...
**Location**: `slote_components/slote_rich_text/example/`
...
**Location**: `slote_components/slote_viewport/example/`
...
**Location**: `slote_components/slote_undo_redo/example/`
...
1. Navigate to the component's example directory:
...
1. Edit files in `slote_components/[component_name]/lib/`
2. The example app will automatically use the updated code (path dependency)
3. Hot reload or restart the example app to see changes

### Adding New Test Features

To add new test features to an example app:
...
Integration testing (e.g., drawing + text overlay, component interactions) should still be done in `slote_app/` where all components are combined. The test platforms focus on individual component functionality.
...
4. **Standard Pattern**: Follows Flutter package conventions (example/ directories)
...
Each example app has minimal dependencies:
...
```yaml
name: slote_[component]_example
...
- Individual component READMEs in each `example/` directory
- Main repository README: `/README.md`
- Component development guide: `slote_app/docs/CONCURRENT_DEVELOPMENT_GUIDE.md`
