...
| **Subscript caret polish (in-run, post–EOT fix)** | After Apr 2026 work, **end-of-paragraph** caret height tracks body vs script much better. **Still open:** collapsed caret **moving from body** into an existing subscript run can draw **too tall**; caret can look **clipped** while **typing** new subscript (general `caretMetrics` path vs EOT). | [`slote_caret_metrics.dart`](../lib/src/appflowy/slote_caret_metrics.dart), [`slote_end_of_paragraph_caret_height.dart`](../lib/src/appflowy/slote_end_of_paragraph_caret_height.dart) — repro: `components/rich_text/example`, `DBG-CARET` logs. |
